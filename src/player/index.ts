...
const CONTROLLER_ID = "onboardtour";
const CONTROLLER_LABEL = "onboardtour";

let id = 0;

const SHELL_SCRIPT_PATTERN = /^>>\s+(.*)$/gm;
const REVERSE_SHELL_SCRIPT_PATTERN = /^> \[([^\]]+)\]\(command:onboardtour\._sendTextToTerminal\?.*$/gm;

export class onboardtourComment implements Comment {
  ...
  constructor(
    ...
    body = body.replace(SHELL_SCRIPT_PATTERN, (_, script) => {
      ...
      return `> [${script}](command:onboardtour._sendTextToTerminal?${args} "Run \\"${script}\\" in a terminal")`;
    });
...
async function renderCurrentStep() {
  ...
  store.activeTour!.thread.comments = [
    new onboardtourComment(step.description, label, store.activeTour!.thread!)
  ];
